...
def test_dark_theme_by_time_and_user_choice():
    ...
    if dark_theme_enabled_by_user is True:
        ...
        # Пользователь выключил темную тему
        is_dark_theme = False
    else:
        # Пользователь не вабрал тему и она устанавливается автоматически по времени суток
        is_dark_theme = current_time.hour >= 22 or current_time.hour < 6
...
def test_find_suitable_user():
    ...
    for user in users:
        if user["age"] < 20:
            suitable_users_20.append(user)
    assert suitable_users_20 == [
        {"name": "Stanislav", "age": 15},
        {"name": "Maria", "age": 18},
    ]
...
"""СОЗДАЕМ ФУНКИЮ, КОТОРАЯ ПРИНИМАЕТ ИМЕНА ФУНКЦИЙ И АРГУМЕНТЫ"""
"""СОЗДАЕМ ПЕРЕМЕННУЮ ФОРМАТИРУЮЩУЮ И СОЕДИНЯЮЩУЮ АРГУМЕНТЫ,
 ВЫВОДИМ НА ПЕЧАТЬ В ЧИТАЕМОМ ВИДЕ"""


def print_function(func, *args):
    result = f"{func.__name__.replace('_', ' ').title()} [{', '.join(str(arg) for arg in args)}]"
    print(result)
    return result
